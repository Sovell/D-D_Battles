...
import { scenarioVictoryXp } from "./core/progression/hero-progression";
...
import { CampaignPanel } from "./app/CampaignPanel";
import { launchCampaignNode } from "./core/campaign/campaign-launch";
import { settleBattleResult } from "./core/campaign/battle-results";
...
export function App() {
  ...
  function launch(config: ScenarioLaunchConfig) {
    if (campaign.pendingReward || campaign.campaignRuns.some((r) => r.pendingBattle)) { setScreen("campaigns"); return; }
    awardingProgression.current = false;
  ...
  function launchWing(selectedRunId: string, nodeId: string) {
    const result = launchCampaignNode(campaign, selectedRunId, nodeId);
    awardingProgression.current = false;
    saveCampaignState(result.campaign);
    setCampaign(result.campaign);
    setRunId(selectedRunId);
    session.loadExpedition({ schemaVersion: 2, savedAt: new Date().toISOString(), seed: result.battle.seed, heroSnapshots: result.battle.heroSnapshots!, state: result.battle });
    setScreen("battle");
  }
  ...
  useEffect(() => {
    ...
    if (state.progressionRewardClaimed && !state.scenario.campaignContext) return;
    if (awardingProgression.current) return;
    awardingProgression.current = true;
    setCampaign((current) => settleBattleResult(state.scenario.campaignContext ? selectParty(current, state.scenario.campaignContext.partyId) : current, state));
    session.claimProgressionReward();
    if (state.scenario.campaignContext) { setRunId(state.scenario.campaignContext.runId); setScreen("campaigns"); }
  ...
  if (screen === "menu") return <MainMenu onCampaigns={() => setScreen("campaigns")} onParty={() => setScreen("party")} onNewScenario={() => setScreen("builder")} onContinue={session.hasSavedSession ? () => { if (state.scenario.campaignContext) setCampaign((c) => selectParty(c, state.scenario.campaignContext!.partyId)); setScreen("battle"); } : undefined} continueSummary={session.hasSavedSession ? `${state.scenario.name} · runda ${state.round} · seed ${session.seed}` : undefined} onLoad={(save) => { const pending = campaign.campaignRuns.find((r) => r.pendingBattle); if (pending && save.state.scenario.campaignContext?.battleId !== pending.pendingBattle!.id) { setScreen("campaigns"); return; } awardingProgression.current = false; if (save.state.scenario.campaignContext) setCampaign((c) => selectParty(c, save.state.scenario.campaignContext!.partyId)); session.loadExpedition(save); setScreen("battle"); }} />;
  if (screen === "party") return <PartyPanel campaign={campaign} onChange={setCampaign} onBack={() => setScreen("menu")} onCampaigns={() => setScreen("campaigns")} />;
  if (screen === "campaigns") return <><CampaignPanel campaign={campaign} onChange={setCampaign} initialRunId={runId} onLaunch={launchWing} onParty={() => setScreen("party")} onBack={() => setScreen("menu")} onContinue={() => { const context = state.scenario.campaignContext; if (context) setCampaign((c) => selectParty(c, context.partyId)); setScreen("battle"); }} />{campaign.pendingReward && <RewardScreen bundle={campaign.pendingReward} onClaim={(itemId) => setCampaign((c) => claimReward(c, itemId))} />}</>;
